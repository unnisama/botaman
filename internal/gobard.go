...
func NewUser(PSID, PSIDTS, CID, RID, ChID, NID, SIDCC, PSIDCC1, PSIDCC3 string) *Bard {
	b := &Bard{
		PSID:    PSID,
		PSIDTS:  PSIDTS,
		answers: make(map[int]*Answer),
	...
	b.answers[b.currAnswer] = &Answer{
		conversationID: CID,
		responseID:     RID,
		choiceID:       ChID,
	}

	for i := 1; i < maxNumOfAnswers; i++ {
		b.answers[i] = &Answer{}
	}
...
func (b *Bard) GetAnswerStruct() *Answer {
	return b.getAnswer(b.currAnswer)
}
...
func (b *Bard) createRestClient() {
	...
	if b.isNewUser {
		...
		cookies = []*http.Cookie{
			...
			{Name: "__Secure-1PSIDCC", Value: b.PSIDCC1},
			{Name: "__Secure-3PSIDCC", Value: b.PSIDCC3},
		}
...
func (b *Bard) doAsk() error {
	...
	for _, cookie := range response.Cookies() {
		...
		case "NID":
			...
			b.SIDCC = ""
		case "SIDCC":
			b.SIDCC = cookie.Value
			b.NID = ""
		case "__Secure-1PSIDCC":
			b.PSIDCC1 = cookie.Value
		case "__Secure-3PSIDCC":
			b.PSIDCC3 = cookie.Value
		}
	}
	// Read the response
